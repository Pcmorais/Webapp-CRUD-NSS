    ...
    public class ChamadosController : Controller
    {
        ...
        public IActionResult Cadastrar(ChamadoViewModel chamadoVM)
        {
            try
            {
                ...
                if (chamadoVM.DataAbertura.Date < DateTime.Now.Date)
                {
                    return BadRequest(new ResponseViewModel("Não é permitido criar/editar chamados com data retroativa."));
                }
                ...
                if (realizadoComSucesso)
                    ...
                    return BadRequest(new ResponseViewModel($"Falha ao gravar o Chamado."));
            }
            catch (Exception ex)
            {
                return BadRequest(new ResponseViewModel(ex));
        ...
        [HttpGet]
        public IActionResult BuscarSolicitantes(string term)
        {
            try
            {
                var chamadosApiClient = new ChamadosApiClient();
                var lstSolicitantes = chamadosApiClient.ChamadosListar().Select(item => item.Solicitante);

                var resultado = lstSolicitantes
                    .Where(s => s.Contains(term, StringComparison.InvariantCultureIgnoreCase))
                    .ToList();
